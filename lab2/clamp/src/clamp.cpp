...
namespace clamp_class {
    ...
    void Clamp::print(std::ostream &s) noexcept {
        s << "type:" << this->get_type() << " signal status:" << this->get_signal() << " link_num:"
          << this->get_link_num() << '\n';
    }
    ...
    void Clamp::operator!() {
        if (status != undefined)
            ...
            throw clamp_exception("can't reverse undefined signal");
    }
    ...
    std::istream& operator >> (std::istream& s, Clamp& obj){
        short type = getNum<short>(s);
        short status = getNum<short>(s);
        unsigned short link_num = getNum<unsigned short>(s);
        obj.set_type(static_cast<clamp_type>(type));
        obj.set_status(static_cast<signal>(status));
        obj.set_link_num(link_num);
        return s;
    }
}
