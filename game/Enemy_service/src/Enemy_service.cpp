...
std::vector<int> Enemy_service::make_shortest_path_mt(const matrix<Cell*>& map,const std::pair<size_t, size_t>& start,
                                                   const std::pair<size_t, size_t>& end)
{   matrix<bool> points;
    std::jthread thread([&](){points = init_for_shortest_path(map);});
    size_t columns = map.get_columns();
    size_t rows = map.get_rows();
    std::vector<bool> visited(columns * rows, false);
    std::vector<int> wave_lengths(columns * rows, -1);
